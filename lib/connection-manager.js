...
ConnectionManager.prototype._receive = function (data) {
    ...
    if (this.remainingBytes > 0) {
        if (data.length < this.remainingBytes) {
            data.copy(this.buffer, this.bufferPosition);
            this.bufferPosition += data.length;
            this.remainingBytes -= data.length;
            position = data.length;
        } else {
            data.copy(this.buffer, this.bufferPosition, 0, this.remainingBytes);
            this.buffers.push(this.buffer);
            position += this.remainingBytes;
            this.buffer = null;
            this.bufferPosition = 0;
            this.remainingBytes = 0;
        }
    ...
    while (position < data.length) {
        ...
        position += 4;
        if ((data.slice(position).length) >= length) {
            this.buffer = data.slice(position, position + length);
            this.buffers.push(this.buffer);
            this.buffer = null;
            position += length;
        } else {
            this.buffer = new Buffer(length);
            data.copy(this.buffer, 0, position);
            this.bufferPosition = data.slice(position).length;
            this.remainingBytes = length - this.bufferPosition;
